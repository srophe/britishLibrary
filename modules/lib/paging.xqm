...
<span xmlns="http://www.w3.org/1999/xhtml">
{
    ...
    return 
        if(request:get-parameter($parameter, '') != '') then
            ...
            else if($parameter = 'syrText') then 
                (<span class="param">Syriac Text: </span>,<span class="match">{request:get-parameter($parameter, '')}&#160;</span>,
                    if(request:get-parameter('syrRubricsLimit', '') = 'true') then
                        <span class="param">Search in Titles/Rubrics</span>
                    else if(request:get-parameter('syrIncipitsLimit', '') = 'true') then
                        <span class="param">Search in Final Rubrics/Subscriptions</span>
                    else if(request:get-parameter('syrColophonsLimit', '') = 'true') then
                        <span class="param">Search in Incipits</span>
                    else if(request:get-parameter('syrExplicitsLimit', '') = 'true') then
                        <span class="param">Search in Explicits/Desinits</span>
                    else if(request:get-parameter('syrOtherLimit', '') = 'true') then
                        <span class="param">Search in Colophons</span>
                    else if(request:get-parameter('syrFinalRubricsLimit', '') = 'true') then
                        <span class="param">Search in Other</span>
                    else if(request:get-parameter('OtherSyrText', '') = 'true') then ()
                    else ()
                )
            else if($parameter = 'title') then 
                (<span class="param">Translated Title: </span>,<span class="match">{request:get-parameter($parameter, '')}&#160;</span>)
            else if($parameter = 'syrRubricsLimit' or 
                $parameter = 'syrIncipitsLimit' or 
                $parameter = 'syrColophonsLimit' or 
                $parameter = 'syrExplicitsLimit' or 
                $parameter = 'syrOtherLimit' or 
                $parameter = 'syrFinalRubricsLimit' or 
                $parameter = 'OtherSyrText') then ()
